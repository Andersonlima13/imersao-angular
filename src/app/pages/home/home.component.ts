...
export class HomeComponent implements OnInit {
  ...
  budgetForm: FormGroup = new FormGroup({
    name: new FormControl('', [Validators.required]),     // agrupar os controles do form de orçamento, ambos como required
  ...
  budgetCategories: BudgetCategory[] = []   // var para armazenar as categorias de orçamento
  budgets: Budget[] = []                     // armazena os orçamentos
  ...
  constructor(
    ...
    private budgetService: BudgetService,    // UserService: Para acessar informações do usuário.
    private expenseService: ExpenseService,  // ExpenseService: Para gerenciar as despesas.
    private router: Router,
    private snackBar: MatSnackBar   // Injeta o MatSnackBar
  ) { }

  ngOnInit(): void {
    this.budgetCategories = this.budgetService.getBudgetsCategories();  // recuperando as categorias que foram adicionadas no orçamento
    this.budgets = this.budgetService.getBudgets();
    this.buildBudgetCards(this.budgets);
    console.log('orcamento carregando: ', this.budgets);   // faz o get dos orçamentos existentes
  ...
  // método para adicionar um orçamento vindo do botão, cria um objeto Budget
  addBudget() {
    const budget: Budget = {
      id: uuidv4(),
  ...
  addExpense() {
    ...
    this.snackBar.open('Gasto adicionado com sucesso!', 'Fechar', {
      duration: 3000, // A duração em milissegundos que o snackbar ficará visível
    });
  ...
  // percorre o array com orçamentos criando um objeto budgetcardconfig.
